...
from ..entities.m12_01_00_00_entities import Flags as m12_01_Flags, PlayerStarts as m12_01_PlayerStarts
...
def Constructor():
    ...
    RunEvent(970, slot=25, args=(17, 2700, 9040, 9030))
    ...
    DeathFollowsWarning()
    RepairWeapons()
    RandomizedMapsLoaded()
    HyperAggressionTimer()
    HealResistances()
    PraiseTheSun()

    PlayerSpeedUp()
    PlayerExtremeSpeedUp()
    PlayerSpeedDown()
    PlayerExtremeSpeedDown()
    PlayerHealthRegain()
    PlayerHealthDrain()
    PlayerMaxHealthUp()
    PlayerMaxHealthDown()
    PlayerStaminaRegenUp()
    PlayerExtremeStaminaRegenUp()
    PlayerStaminaRegenDown()
    PlayerMaxEquipLoadUp()
    PlayerMaxEquipLoadDown()
    PlayerDefenseUp()
    PlayerInvincible()
    PlayerDefenseDown()
    PlayerInstantDeath()
    ...
    PlayerPainfulHeals()
    PlayerEggInfection()
    PlayerBinocularsMode()
    PlayerThorny()
    PlayerFallControl()
    PlayerFireWeapon()
    PlayerMagicWeapon()
    PlayerLightningWeapon()
    PlayerFullHeal()
    PlayerOneHPPreHeal()
    PlayerSoulBoost()
    PlayerSoulSteal()
...
def HookCheck():
    ...
    message. """
    EnableFlag(SystemFlags.HookCheck)
    Wait(5.0)  # Game manager will disable flag if running.
    if SystemFlags.HookCheck:
        # Display error message and disable all pending request flags.
        DisplayBattlefieldMessage(CommonText.HookConnectionError, 0)
    Wait(5.0)
    return RESTART


def WarpToManus():
    """ 1900: Warp directly to Manus.

    Only works if fight is not active and the player has left the tutorial. Awards some bonus souls instead if Manus is
    already dead.

    Event 11212099 in m12_01_00_00 manages the fight flags and disables the request flag.
    """
    Await(
        FlagEnabled(11810000)
        and FlagDisabled(11215022)
        and FlagEnabled(TriggerFlags.SurpriseManusFight)
    )
    DisableFlag(TriggerFlags.SurpriseManusFight)

    if FlagEnabled(17):
        # Manus is dead.
        DisplayStatus(CommonText.ManusDefeated)
        Wait(1.0)
        AddSpecialEffect(PLAYER, CommonEffects.ManusReward)
        return RESTART
    else:
        # Trigger flags is disabled in m12_01 script.
        EnableFlag(m12_01_Flags.SurpriseManusFightActive)
        WarpToMap(OOLACILE, m12_01_PlayerStarts.c0000_ManusWarp)
        # No point restarting.


def GetArrows():
    """ 1901: Give player 99 Large Arrows. Triggered when Archer set is force-equipped in C#. """
    Await(TriggerFlags.RequestArrows)
    DisableFlag(TriggerFlags.RequestArrows)
    AwardItemLot(CommonItemLots.ArrowGift)
    return RESTART


def DeathFollowsWarning():
    """ 1903: Displays a warning message before the lava effect begins in C#. """
    Await(TriggerFlags.RequestDeathFollowsWarning)
    DisableFlag(TriggerFlags.RequestDeathFollowsWarning)
    DisplayBattlefieldMessage(CommonText.DeathFollowsWarning, 0)
    return RESTART


def RepairWeapons():
    """ 1905: Repair right and left hand weapons. """
    Await(TriggerFlags.RequestRepair)
    DisableFlag(TriggerFlags.RequestRepair)
    AddSpecialEffect(PLAYER, 3100)
    AddSpecialEffect(PLAYER, 3101)
    return RESTART


def RandomizedMapsLoaded():
    """ 1907: Listens for C#-enabled flag that indicates maps have been randomized, and enables a flag that causes the
    C# reset timer to begin (so we can guarantee the randomized maps are seen at least once). """
    if not TriggerFlags.MapsAreRandomized or InsideMap(UNDEAD_ASYLUM) or InsideMap(FIRELINK_SHRINE):
        return
    EnableFlag(TriggerFlags.RandomizedMapsLoaded)


def HyperAggressionTimer():
    """ 1909: Timer for hyper aggression that survives resting at a bonfire (as resting naturally resets AI params).

    When hyper aggression is requested, this event enables an additional flag that will cause the hyper aggression
    events to immediately trigger. This flag is only disabled on map reload, not resting at a bonfire, so resting will
    not end the effect (as the events will immediately fire again).

    However, this event disables the hyper aggression trigger itself after 30 seconds, so it will not run again as long
    as the map stays loaded for at least that long.
    """
    DisableFlag(TriggerFlags.HyperAggressionReTrigger)
    Await(TriggerFlags.HyperAggression)
    EnableFlag(TriggerFlags.HyperAggressionReTrigger)  # will immediately trigger hyper aggression again (e.g. on rest)
    Wait(30.0)
    DisableFlag(TriggerFlags.HyperAggression)  # hyper aggression will now be disabled on reload, warp, death, etc.
    Wait(60.0)
    return RESTART  # re-trigger flag will be disabled again


def HealResistances():
    """ 1911: Heal all resistances. """
    Await(TriggerFlags.RequestHealResistances)
    DisableFlag(TriggerFlags.RequestHealResistances)
    WaitFrames(1)  # allow time for cosplay equipment
    AddSpecialEffect(PLAYER, 3051)  # bleed
    AddSpecialEffect(PLAYER, 3061)  # poison
    AddSpecialEffect(PLAYER, 3071)  # toxic
    AddSpecialEffect(PLAYER, 3090)  # curse
    return RESTART


def PraiseTheSun():
    """ 1913: Force a 'Praise the Sun' animation, anytime, anywhere. """
    Await(TriggerFlags.PraiseTheSun)
    DisableFlag(TriggerFlags.PraiseTheSun)
    ForceAnimation(PLAYER, 6801)
    return RESTART


def PlayerSpeedUp():
    """ 11812100: Temporary player effect. """
    Await(TriggerFlags.PlayerSpeedUp)
    DisableFlag(TriggerFlags.PlayerSpeedUp)
    AddSpecialEffect(PLAYER, 100000)
    return RESTART


def PlayerExtremeSpeedUp():
    """ 11812101: Temporary player effect. """
    Await(TriggerFlags.PlayerExtremeSpeedUp)
    DisableFlag(TriggerFlags.PlayerExtremeSpeedUp)
    AddSpecialEffect(PLAYER, 100001)
    return RESTART


def PlayerSpeedDown():
    """ 11812102: Temporary player effect. """
    Await(TriggerFlags.PlayerSpeedDown)
    DisableFlag(TriggerFlags.PlayerSpeedDown)
    AddSpecialEffect(PLAYER, 100002)
    return RESTART


def PlayerExtremeSpeedDown():
    """ 11812103: Temporary player effect. """
    Await(TriggerFlags.PlayerExtremeSpeedDown)
    DisableFlag(TriggerFlags.PlayerExtremeSpeedDown)
    AddSpecialEffect(PLAYER, 100003)
    return RESTART


def PlayerHealthRegain():
    """ 11812104: Temporary player effect. """
    Await(TriggerFlags.PlayerHealthRegain)
    DisableFlag(TriggerFlags.PlayerHealthRegain)
    AddSpecialEffect(PLAYER, 100004)
    return RESTART


def PlayerHealthDrain():
    """ 11812105: Temporary player effect. """
    Await(TriggerFlags.PlayerHealthDrain)
    DisableFlag(TriggerFlags.PlayerHealthDrain)
    AddSpecialEffect(PLAYER, 100005)
    return RESTART


def PlayerMaxHealthUp():
    """ 11812106: Temporary player effect. """
    Await(TriggerFlags.PlayerMaxHealthUp)
    DisableFlag(TriggerFlags.PlayerMaxHealthUp)
    AddSpecialEffect(PLAYER, 100006)
    return RESTART


def PlayerMaxHealthDown():
    """ 11812107: Temporary player effect. """
    Await(TriggerFlags.PlayerMaxHealthDown)
    DisableFlag(TriggerFlags.PlayerMaxHealthDown)
    AddSpecialEffect(PLAYER, 100007)
    return RESTART


def PlayerStaminaRegenUp():
    """ 11812108: Temporary player effect. """
    Await(TriggerFlags.PlayerStaminaRegenUp)
    DisableFlag(TriggerFlags.PlayerStaminaRegenUp)
    AddSpecialEffect(PLAYER, 100008)
    return RESTART


def PlayerExtremeStaminaRegenUp():
    """ 11812109: Temporary player effect. """
    Await(TriggerFlags.PlayerExtremeStaminaRegenUp)
    DisableFlag(TriggerFlags.PlayerExtremeStaminaRegenUp)
    AddSpecialEffect(PLAYER, 100009)
    return RESTART


def PlayerStaminaRegenDown():
    """ 11812110: Temporary player effect. """
    Await(TriggerFlags.PlayerStaminaRegenDown)
    DisableFlag(TriggerFlags.PlayerStaminaRegenDown)
    AddSpecialEffect(PLAYER, 100010)
    return RESTART


def PlayerMaxEquipLoadUp():
    """ 11812111: Temporary player effect. """
    Await(TriggerFlags.PlayerMaxEquipLoadUp)
    DisableFlag(TriggerFlags.PlayerMaxEquipLoadUp)
    AddSpecialEffect(PLAYER, 100011)
    return RESTART


def PlayerMaxEquipLoadDown():
    """ 11812112: Temporary player effect. """
    Await(TriggerFlags.PlayerMaxEquipLoadDown)
    DisableFlag(TriggerFlags.PlayerMaxEquipLoadDown)
    AddSpecialEffect(PLAYER, 100012)
    return RESTART


def PlayerDefenseUp():
    """ 11812113: Temporary player effect. """
    Await(TriggerFlags.PlayerDefenseUp)
    DisableFlag(TriggerFlags.PlayerDefenseUp)
    AddSpecialEffect(PLAYER, 100013)
    return RESTART


def PlayerInvincible():
    """ 11812114: Temporary player effect. """
    Await(TriggerFlags.PlayerInvincible)
    DisableFlag(TriggerFlags.PlayerInvincible)
    AddSpecialEffect(PLAYER, 100014)
    return RESTART


def PlayerDefenseDown():
    """ 11812115: Temporary player effect. """
    Await(TriggerFlags.PlayerDefenseDown)
    DisableFlag(TriggerFlags.PlayerDefenseDown)
    AddSpecialEffect(PLAYER, 100015)
    return RESTART


def PlayerInstantDeath():
    """ 11812116: Temporary player effect. """
    Await(TriggerFlags.PlayerInstantDeath)
    DisableFlag(TriggerFlags.PlayerInstantDeath)
    DisplayBattlefieldMessage(CommonText.InstantDeathWarning, 0)
    Wait(2.0)
    AddSpecialEffect(PLAYER, 100016)
    return RESTART


def PlayerExterminate():
    """ 11812117: Temporary player effect. """
    Await(TriggerFlags.PlayerExterminate)
    DisableFlag(TriggerFlags.PlayerExterminate)
    AddSpecialEffect(PLAYER, 100017)
    return RESTART


def PlayerDetectionDown():
    """ 11812118: Temporary player effect. """
    Await(TriggerFlags.PlayerDetectionDown)
    DisableFlag(TriggerFlags.PlayerDetectionDown)
    AddSpecialEffect(PLAYER, 100018)
    return RESTART


def PlayerSoulDrain():
    """ 11812119: Temporary player effect. """
    Await(TriggerFlags.PlayerSoulDrain)
    DisableFlag(TriggerFlags.PlayerSoulDrain)
    AddSpecialEffect(PLAYER, 100019)
    return RESTART


def PlayerSturdy():
    """ 11812120: Temporary player effect. """
    Await(TriggerFlags.PlayerSturdy)
    DisableFlag(TriggerFlags.PlayerSturdy)
    AddSpecialEffect(PLAYER, 100020)
    return RESTART


def PlayerFragile():
    """ 11812121: Temporary player effect. """
    Await(TriggerFlags.PlayerFragile)
    DisableFlag(TriggerFlags.PlayerFragile)
    AddSpecialEffect(PLAYER, 100021)
    return RESTART


def PlayerWeakHeals():
    """ 11812122: Temporary player effect. """
    Await(TriggerFlags.PlayerWeakHeals)
    DisableFlag(TriggerFlags.PlayerWeakHeals)
    AddSpecialEffect(PLAYER, 100022)
    return RESTART


def PlayerPainfulHeals():
    """ 11812123: Temporary player effect. """
    Await(TriggerFlags.PlayerPainfulHeals)
    DisableFlag(TriggerFlags.PlayerPainfulHeals)
    AddSpecialEffect(PLAYER, 100023)
    return RESTART


def PlayerEggInfection():
    """ 11812124: Temporary player effect. """
    Await(TriggerFlags.PlayerEggInfection)
    DisableFlag(TriggerFlags.PlayerEggInfection)
    AddSpecialEffect(PLAYER, 100024)
    return RESTART


def PlayerBinocularsMode():
    """ 11812125: Temporary player effect. """
    Await(TriggerFlags.PlayerBinocularsMode)
    DisableFlag(TriggerFlags.PlayerBinocularsMode)
    AddSpecialEffect(PLAYER, 100025)
    return RESTART


def PlayerThorny():
    """ 11812126: Temporary player effect. """
    Await(TriggerFlags.PlayerThorny)
    DisableFlag(TriggerFlags.PlayerThorny)
    AddSpecialEffect(PLAYER, 100026)
    return RESTART


def PlayerFallControl():
    """ 11812127: Temporary player effect. """
    Await(TriggerFlags.PlayerFallControl)
    DisableFlag(TriggerFlags.PlayerFallControl)
    AddSpecialEffect(PLAYER, 100027)
    return RESTART


def PlayerFireWeapon():
    """ 11812128: Temporary player effect. """
    Await(TriggerFlags.PlayerFireWeapon)
    DisableFlag(TriggerFlags.PlayerFireWeapon)
    AddSpecialEffect(PLAYER, 100028)
    return RESTART


def PlayerMagicWeapon():
    """ 11812129: Temporary player effect. """
    Await(TriggerFlags.PlayerMagicWeapon)
    DisableFlag(TriggerFlags.PlayerMagicWeapon)
    AddSpecialEffect(PLAYER, 100029)
    return RESTART


def PlayerLightningWeapon():
    """ 11812130: Temporary player effect. """
    Await(TriggerFlags.PlayerLightningWeapon)
    DisableFlag(TriggerFlags.PlayerLightningWeapon)
    AddSpecialEffect(PLAYER, 100030)
    return RESTART


def PlayerFullHeal():
    """ 11812131: Temporary player effect. """
    Await(TriggerFlags.PlayerFullHeal)
    DisableFlag(TriggerFlags.PlayerFullHeal)
    AddSpecialEffect(PLAYER, 100031)
    return RESTART


def PlayerOneHPPreHeal():
    """ 11812132: Temporary player effect. """
    Await(TriggerFlags.PlayerOneHPPreHeal)
    DisableFlag(TriggerFlags.PlayerOneHPPreHeal)
    AddSpecialEffect(PLAYER, 100032)
    return RESTART


def PlayerSoulBoost():
    """ 11812133: Temporary player effect. """
    Await(TriggerFlags.PlayerSoulBoost)
    DisableFlag(TriggerFlags.PlayerSoulBoost)
    AddSpecialEffect(PLAYER, 100033)
    return RESTART


def PlayerSoulSteal():
    """ 11812134: Temporary player effect. """
    Await(TriggerFlags.PlayerSoulSteal)
    DisableFlag(TriggerFlags.PlayerSoulSteal)
    AddSpecialEffect(PLAYER, 100034)
    return RESTART
